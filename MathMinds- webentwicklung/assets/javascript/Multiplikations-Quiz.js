document.addEventListener("DOMContentLoaded", function() {
    /* Funktion zum Generieren einer zufälligen Multiplikationsaufgabe */
    function generateMultiplicationQuestion() {
        const num1 = Math.floor(Math.random() * 10) + 1; /* Zahl zwischen 1 und 10 */
        const num2 = Math.floor(Math.random() * 10) + 1; /* Zahl zwischen 1 und 10 */
        return { num1, num2, answer: num1 * num2 };
    }

    /* Funktion zum Anzeigen mehrerer Multiplikationsaufgaben */
    function displayMultiplicationQuestions() {
        ...
        for (let i = 0; i < 6; i++) { /* 5 Aufgaben gleichzeitig anzeigen */
    ...
    function checkMultiplicationAnswers() {
        ...
        setTimeout(displayMultiplicationQuestions, 6000);
    }

    /* Initialisierung des Multiplikations-Quizzes */
    const solveButton = document.getElementById("solveButton");
    if (solveButton) {
        solveButton.addEventListener("click", checkMultiplicationAnswers);
    }

    /* Zeige die ersten Multiplikationsfragen */
    displayMultiplicationQuestions();
});
